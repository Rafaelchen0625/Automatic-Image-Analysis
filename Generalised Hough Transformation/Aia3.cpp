...
void Aia3::plotHough(vector< vector<Mat> >& houghSpace){
    ...
    showImage(maxImage,"hough space", 0);
    Mat spectrum;
    normalize(maxImage, spectrum, 0, 255, CV_MINMAX);
    spectrum.convertTo(spectrum, CV_8UC1);
    imwrite("houghspace.png", spectrum);
}
...
void Aia3::makeFFTObjectMask(vector<Mat>& templ, double scale, double angle, Mat& fftMask){
    //OI(x,y)
    Mat gradients[] = {Mat::zeros(templ[1].rows,templ[1].cols,CV_32FC1),
                       Mat::zeros(templ[1].rows,templ[1].cols,CV_32FC1)};
    split(templ[1], gradients);
    
    //Normalize the magnitudes of complex gradients
    Mat gradMagnitude;
    cv::magnitude(gradients[0], gradients[1], gradMagnitude);
    cv::Scalar magSum = cv::sum(gradMagnitude);
    cv::divide(gradients[0], magSum, gradients[0]);
    cv::divide(gradients[1], magSum, gradients[1]);
    
    //Binary object mask and complex object gradients are rotated and scaled
    Mat binMask = rotateAndScale(templ[0],angle,scale);
    Mat gradientX = rotateAndScale(gradients[0], angle, scale);
    Mat gradientY = rotateAndScale(gradients[1], angle, scale);
    
    //rotated gradients by multiplication exp(theta)
    Mat magnitude, phase;
    cartToPolar(gradientX, gradientY, magnitude, phase);
    cv::add(phase, Scalar(angle), phase);
    polarToCart(magnitude, phase, gradientX, gradientY);

    //OI(x,y)OB(x,y)
    multiply(gradientX,binMask,gradientX);
    multiply(gradientY,binMask,gradientY);
    
    //centering
    Mat newMask[] = {Mat::zeros(fftMask.rows, fftMask.cols, CV_32FC1),
                     Mat::zeros(fftMask.rows, fftMask.cols, CV_32FC1)};
    gradientX.copyTo(newMask[0](cv::Rect(0,0,gradientX.cols,gradientX.rows)));
    gradientY.copyTo(newMask[1](cv::Rect(0,0,gradientY.cols,gradientY.rows)));
    
    int cx = gradientX.cols/2;
    int cy = gradientX.rows/2;
    Mat newTempl;
    merge(newMask,2,newTempl);
    circShift(newTempl, newTempl, -cx, -cy);
    
    cv::dft(newTempl,fftMask);
}
...
vector< vector<Mat> > Aia3::generalHough(Mat& gradImage, vector<Mat>& templ, double scaleSteps, double* scaleRange, double angleSteps, double* angleRange){
    ...
    double scale,angle;;
    double scaleFactor = (scaleRange[1]-scaleRange[0])/(scaleSteps-1);
    double angleFactor = (angleRange[1]-angleRange[0])/angleSteps;
    
    for (int s = 0; s < int(scaleSteps); s++) {
        scale = scaleRange[0] + scaleFactor * s;
        vector<Mat> houghAngles;
        for (int a = 0; a < int(angleSteps); a++) {
            angle = angleRange[0] + angleFactor * a;
            
            Mat fftMask = Mat::zeros(gradImage.rows, gradImage.cols, CV_32FC2);
            makeFFTObjectMask(templ,scale,angle,fftMask);
             
            //Multiply spectrum of the result
            Mat query, correlation;
            cv::dft(gradImage, query);
            cv::mulSpectrums(query, fftMask, correlation, 0, true);
            
            cv::idft(correlation, query);
            Mat coordinates[] = {Mat::zeros(query.rows,query.cols, CV_32FC1),
                                 Mat::zeros(query.rows,query.cols, CV_32FC1)};
            split(query, coordinates);
            Mat magI;
            magnitude(coordinates[0], coordinates[1], magI);
            
            houghAngles.push_back(magI);
        }
        hough.push_back(houghAngles);
    }
...
vector<Mat> Aia3::makeObjectTemplate(Mat& templateImage, double sigma, double templateThresh){
    ...
    // calculate the binary edge mask
    Mat gradients[] = {Mat::zeros(gradImage.rows, gradImage.cols, CV_32FC1),
                       Mat::zeros(gradImage.rows,gradImage.cols, CV_32FC1)};
    split(gradImage, gradients);
    showImage(gradients[0], "gradient x", 0);
    showImage(gradients[1], "gradient y", 0);
    Mat gradMagnitude, gradAngel;
    cartToPolar(gradients[0], gradients[1], gradMagnitude, gradAngel);
    
    double gradMagMax, gradMagMin = 0.0;
    minMaxLoc(gradMagnitude, &gradMagMin, &gradMagMax);
    
    //OB(x,y)=|OI(x,y)|>(Thresh*max{|OI(x,y)|})
    double dThresh = templateThresh * gradMagMax;
    Mat templateMask = Mat::zeros(gradMagnitude.rows, gradMagnitude.cols, CV_32FC1);
    for (int r = 0; r < gradMagnitude.rows; r++) {
        for (int c = 0; c < gradMagnitude.cols; c++) {
            if (gradMagnitude.at<float>(r,c) > dThresh) {
                templateMask.at<float>(r,c) = gradMagnitude.at<float>(r,c);
            }
        }
    }

    vector<Mat> vGradients;
    vGradients.push_back(templateMask);
    vGradients.push_back(gradImage);
    return vGradients;
}
...
void Aia3::run(string tmplImg, string testImg){
    ...
    double objThresh 		= 0.505;	// relative threshold for maxima in hough space
    double scaleSteps 		= 8;		// scale resolution in terms of number of scales to be investigated
